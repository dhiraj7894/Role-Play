...
public class Recorder : MonoBehaviour, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    public static event EventHandler OnRecordingEventStart;
    ...
    {
        if (holdToRecord && !RecordTimer.Instance.GetRecordingStatus)
            StartRecording();
    }
    ...
    {                
        if (isRecording)
        {
            ...
            //// Showing and Storing the audio file to Audio Manager List
            audioClip.name = fileName + " " + AudioFileNumber;
            if (AudioManager.Instance.audioClips.Count > 0)
            {
                if (AudioManager.Instance.audioClips[AudioManager.Instance.audioClips.Count - 1].name == audioClip.name)
                {
                    Debug.Log("Dublicate Audio Files Saved");
                    AudioManager.Instance.audioClips[AudioManager.Instance.audioClips.Count - 1] = audioClip;
                }
                else if (AudioManager.Instance.audioClips[AudioManager.Instance.audioClips.Count - 1].name != audioClip.name)
                {
                    Debug.Log("New Audio Files Saved");
                    AudioManager.Instance.audioClips.Add(audioClip);
                    
                }                                
            }
            else
            {
                Debug.Log("First Audio Files Saved");
                AudioManager.Instance.audioClips.Add(audioClip);                
            }
            AudioManager.Instance.UpdateVisual();



            string filePath = Path.Combine(Application.persistentDataPath,audioClip.name+".wav"/*"A" + fileName + " " + AudioFileNumber + ".wav"*/);
            ...
            try
            {
                ...
                RecordTimer.Instance.StopTimer();
    ...
    public void OnRecordingStart()
    {
        ...
        AudioName = VideoManager.Instance.AudioName;
        AudioFileNumber = VideoManager.Instance.AudioFileNumber;
    }
    ...
    public void GetAudioFileNumber(int i)
    {
        AudioFileNumber = i;
    }   
}